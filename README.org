...
Let's say want to stop using the Control key altogether in favour of Super.
...
I.e. keep referring to "C-..." instead of "s-...".

One advantage of doing so is that Control and Super perfectly mirror each other, useful in the event Super has a service outage for any reason, like you install GNOME or you enter the TTY, or you plug in on a keyboard that lacks a Right Super key but has a Right Control, or vice versa.
...
When you develop a package like this, you run into many gotchas.  I had to learn not to utilize command remappings such as =(define-key map [remap old-command] new-command)= nor key translations such as =(define-key key-translation-map (kbd "C-c") (kbd "s-c"))= even though these solutions seem clean.  There are always edge cases.

Instead, Massmapper just scans each and every keymap variable, such as =global-map=, =org-mode-map= and =projectile-mode-map=, and makes new key-bindings inside each, based on other key-bindings found within.  Brute-force, but works as intended 100% of the time.  The only drawback is cluttering up the which-key popup.
...
I surmise we can't remove =C-g= itself because of TTYs, but it should be possible to add /more/ keyboard interrupts.  Emacs would probably need a compile-time option, which could maybe become a startup parameter or Lisp setting.

There is nothing Massmapper can do from within Emacs Lisp.

*** Gotcha: C-g is sometimes a hardcoded "cancel-button"
Aside from the above, some Emacs Lisp functions hardcode =C-g= as a "cancel" action, but this can usually be circumvented with a key-translation---which for your information, Massmapper won't do by default.  Basically this:

#+begin_src elisp
(define-key input-decode-map (kbd "s-g") (kbd "C-g"))
#+end_src

In very rare cases, mainly =map-y-or-n-p= that I've seen, they [[https://lists.gnu.org/archive/html/bug-gnu-emacs/2022-10/msg02195.html][actually use read-event]] so that key-translations do nothing!  That's properly considered a bug and should be reported upstream.
...
and if you must refer directly to a key in some custom interaction, definitely don't use =read-event=, =read-char= or =read-char-exclusive=, because they don't respect key translations!  These are demon functions that should trigger warnings in package-lint and the like.
...
Note that the mentioned =my-keyboard-quit-wrapper= should probably call =keyboard-quit= itself at the end.  As an alternative, you can try to design your program in such a way that when a keyboard-quit comes, the right thing would happen automatically, but that's more art than science.
