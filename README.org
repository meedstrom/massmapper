...
# Maybe in the future, it'll also be able to free up C-g and C-[, but don't count on it; those are complicated affairs.
...
If you must refer directly to a key in some custom interaction, don't use the demon functions =read-event=, =read-char= or =read-char-exclusive=, because they don't respect key translations!
