...
Let's say you plan to stop using the Control key altogether in favour of Super.
...
An advantage is that you keep Control useful in the event Super has a service outage for any reason (like you install GNOME or you're in the TTY), or you get on a keyboard that lacks a Right Super key but has a Right Control, or vice versa.
...
*** C-g is a hardcoded "unfreeze Emacs"
...
*** C-g is sometimes a hardcoded "cancel-button"
...
and definitely don't use =read-event=, =read-char= or =read-char-exclusive=, because they don't respect key translations.  These are demon functions that should trigger warnings in package-lint and the like.
...
Note that your function =my-keyboard-quit-wrapper= should probably call =keyboard-quit= itself at the end.  As an alternative, you can try to design your program in such a way that when a keyboard-quit comes, the right thing would happen automatically, but that's more art than science.
