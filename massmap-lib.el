;;; massmap-lib.el -- Library for deianira and massmap -*- lexical-binding: t -*-

;; (read-symbol-shorthands . '("massmap-" . "massmap-lib-"))
...
(defconst massmap--ignore-keys-control-chars
  '("ESC" "C-["
    "RET" "C-m"
    "TAB" "C-i"))

(defconst massmap--ignore-keys-irrelevant
...
(defun massmap--raw-keymap-recursive (map)
  "If MAP is a keymap, return it; if a symbol, evaluate first.
...
(defvar massmap--unnest-avoid-prefixes
  (cons "C-x 8"
...
(defun massmap--last-key (single-key-or-chord)
...
(defun massmap--key-contains-multi-chord (keydesc)
...
Assumes KEYDESC was normalized by (key-description (kbd KEY))."
  ...
  (string-match-p (rx (= 2 (regexp massmap--modifier-regexp)))
                  keydesc))

(defun massmap--key-contains (symlist keydesc)
...
`massmap--all-shifted-symbols-list' as SYMLIST."
  (declare (pure t) (side-effect-free t))
  (->> (split-string keydesc " ")
       (-map #'massmap--last-key)
       (-intersection symlist)))

(defun massmap--key-has-more-than-one-chord (keydesc)
...
that, see `massmap--key-contains-any'."
  (declare (pure t) (side-effect-free t))
  (when-let ((first-modifier-pos
              (string-match-p massmap--modifier-regexp-safe keydesc)))
    ;; We use +1 and not +2 b/c of the peculiarities of this regexp, but it
    ;; gets the job done.
    (string-match-p massmap--modifier-regexp-safe keydesc (1+ first-modifier-pos))))

(defun massmap--key-starts-with-modifier (keydesc)
...
(defun massmap--key-mixes-modifiers (keydesc)
  "Return t if KEYDESC has more than one kind of modifier.
Does not catch shiftsyms such as capital letters; to check for
those, see `massmap--key-contains-any'.  Does catch e.g. C-S-<RET>."
  (declare (pure t) (side-effect-free t))
  (let ((case-fold-search nil)
        (first-match-pos (string-match-p massmap--modifier-regexp-safe keydesc)))
    (when first-match-pos
      (let* (;; Compensate if `massmap--modifier-regexp-safe' matched a dash or
...
(defun massmap--key-seq-steps=1 (keydesc)
        ...
        (not (cl-loop
              ...
              unless (and
                      ...
                      (string-prefix-p root-modifier
                                       (substring step 0 2))
                      (not (string-match-p massmap--modifier-regexp-safe
                                           (substring step 2))))
              return t))))))

(defun massmap--stem-to-parent-keydesc (stem)
...
example, inputting a STEM of \"C-x \" returns \"C-x\"."
  ...
  (if (string-suffix-p " " stem)
      (substring stem 0 -1)
    (if (string-match-p (concat massmap--modifier-regexp "$") stem)
        (replace-regexp-in-string (rx " " (+ (regexp massmap--modifier-regexp)) eol)
                                  ""
                                  stem)
      (error "Non-stem passed to `massmap--stem-to-parent-keydesc': %s" stem))))

(defun massmap--get-leaf (keydesc)
...
(defun massmap--drop-leaf (keydesc)
  "Chop the leaf off KEYDESC and return the resulting stem."
  (declare (pure t) (side-effect-free t))
  (replace-regexp-in-string (rx (literal (massmap--get-leaf keydesc)) eol)
                            ""
                            keydesc))

(defun massmap--parent-stem (stem)
...
sub-keymap.  As such, both of these return the same parent: \"C-\"."
  (declare (pure t) (side-effect-free t))
  (if (or (= 2 (length stem))
          (massmap--key-seq-steps=1 stem))
      nil
    (massmap--drop-leaf (massmap--stem-to-parent-keydesc stem))))

(defun massmap--parent-key (keydesc)
...
(defun massmap--prefix-to-stem (keydesc)
